...
const ForgotPassword = ({ showImage = true }) => {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "/api/Customers/sendOtptoUpdatePassword",
        {
    ...
    if (/^\d{4}$/.test(values.otp)) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          message.success(
            "Otp Verfied, Now Updating your Password, Hold tight."
          );
          const requestBody = {
          ...
          const responseUpdate = await fetch(
            `/api/Customers/UpdateCustomerPassword`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  return (
    <div className="flex items-center justify-center w-full h-[100svh]">
      <div className="lg:w-1/2 xl:mx-auto xl:w-full xl:max-w-sm 2xl:max-w-md">
        <Link href={`${HomeUrl}`}>
          <Image
            src={logo}
