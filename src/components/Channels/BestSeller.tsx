...
const BestSeller = () => {
    ...
    const [filteredCategories, setFilteredCategories] = React.useState(categories); // For filtered categories
            ...
            try {
                ...
                if (activeTab === 1) {
                    ...
                    filteredProducts = response.filter(
                        (product: any) =>
                            new Date(product.updated_at_product) > last30Days
                    );
                } else if (activeTab === 2) {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                ...
                const categoriesWithRecommended = [{ category_name: 'Recommended' }, ...response];
        ...
        <div className='px-2 md:px-10 mt-4'>
            <div className='md:flex items-center md:justify-between'>
                <div className='md:flex items-center'>
                    <h1 className='text-sm font-semibold mr-2  text-gray-600'>Best Sellers</h1>
                    <Tabs activeTab={activeTab} onTabClick={onTabClick} mode='scrollSelectedToCenterFromView' >
                ...
                <div className='md:flex py-2 justify-center items-center'>
                    <h1 className='text-sm mb-2 md:mb-0 font-semibold mr-2 text-gray-600' >Filter by category</h1>
                    <Menu
                        dismiss={{
                            itemPress: false,
                        }}
                    ...
                    >
                        ...
                        <MenuList className="max-h-[50vh] scrollbar-hidden">
                            ...
                            {filteredCategories.map((item: any, i: number) => (
                                <MenuItem key={i} className={`${selectedCatgory === item.category_name && 'bg-gray-300 text-black'}`} onClick={() => setSelectedCatgeory(item.category_name)
