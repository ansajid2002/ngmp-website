...
const LikeButton: React.FC<LikeButtonProps> = ({
  ...
  const handleClicked = () => {
    setLikeButton((prevLikeButton: any) => !prevLikeButton);
  };

  useEffect(() => {
    // Update the local state when the liked prop changes
    setLikeButton(liked);
  }, [liked]);
    ...
    >
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
        <path
          ...
          stroke={likeButton ? "#ef4444" : "currentColor"}
          fill={likeButton ? "#ef4444" : "none"}
          strokeWidth="1.5"
          strokeLinecap="round"
