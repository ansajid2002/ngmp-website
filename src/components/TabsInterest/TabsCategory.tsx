...
const TabsCategory = ({ categoriesdata }) => {
    const allSubcategories = categoriesdata.flatMap((category: any) => category.subcategories);
    const [activeTab, setActiveTab] = React.useState(0);
    const [subcatname, setSubcatname] = React.useState(allSubcategories[0].subcategory_name);
        ...
        const fetchSubcategoryProducts = async () => {
            try {
                ...
                const replacesubcategory = subcatname
                    .replace(/[^\w\s]/g, "")
                    .replace(/\s/g, "");
                const response = await fetch(`/api/subcatgeoryProducts/${replacesubcategory}`);
    ...
    return (
        <>
            <Tabs
                activeTab={activeTab}
                onTabClick={(e, index) => onTabClick(e, index, allSubcategories[index].subcategory_name)}
            ...
            <TabScreen key={activeTab} activeTab={activeTab} index={activeTab}>
                {
                    loading ?
                        ...
                        :
                        <div className="grid grid-cols-2 gap-6 md:gap-8 sm:grid-cols-3 lg:grid-cols-5 px-2 mt-10">
                            {products && products.map((subcat, index) => (
