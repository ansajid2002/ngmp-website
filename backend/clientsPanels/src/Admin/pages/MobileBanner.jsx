...
const MobileBanner = () => {
    ...
    const renderFilePondInstances = () => {
        return filePondInstances.map((instanceData, index) => (
            <div key={index} className='md:w-[40%] '>
                <h2 className='mt-4  text-2xl font-semibold mb-2'>Mobile Home Banner Upload - #{index + 1}</h2>
                <FilePond
                ...
                <div className='h-[80px]'>
                    {banners[index] && banners[index].mobilebanner_url && (
                        <>
                            <div className='flex  items-center'>
                                <Image
                                    width={50}
                                    src={`${AdminUrl}/uploads/MobileBanners/${banners[index].mobilebanner_url}`}
    ...
    const handleUpload = (instanceIndex) => {
        ...
        const backendUploadUrl = `${AdminUrl}/api/uploadMobileBanner`;
    ...
    const handleDelete = (instanceIndex) => {
        ...
        if (bannerId) {
            ...
            axios
                .delete(`${AdminUrl}/api/deleteMobileBanner/${bannerId}`)
                .then(() => {
