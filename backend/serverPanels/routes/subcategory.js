...
app.post("/addNewSubCategories", async (req, res) => {
  try {
    ...
    const { subcategory_name, subcategory_description } =
      req.body.values;
...
const imgConfigSubcatgeory = multer.diskStorage({
  destination: (req, file, callback) => {
    console.log(file);
    callback(null, "./uploads/SubcategoryImages");
  },
...
app.post('/UploadSubcatgeoryImage', uploadSubcategory.single('file'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const query = 'SELECT subcategory_image_url FROM subcategories WHERE subcategory_id = $1';
    ...
    if (oldImageUrl) {
      const oldImageFilePath = `./uploads/SubcategoryImages/${oldImageUrl}`;
    ...
    const updateQuery = 'UPDATE subcategories SET subcategory_image_url = $1 WHERE subcategory_id = $2';
    const values = [file.filename, subcategoryId]; // Adjust as per your needs

    await pool.query(updateQuery, values);

    res.status(200).json({ file: file.filename, message: 'SubCategory image updated successfully.' });
  } catch (error) {
    console.error(error);
