...
app.post("/SetAttributesValues", async (req, res) => {
    try {
        const { attributeName, attributeValues, type, category, subcategory, backendCategory, backendSubCategory } = req.body;
        ...
        if (type === 'update') {
            ...
            `;
            const { rows: updatedRows } = await pool.query(updateQuery, [attributeValues, JSON.stringify(category), JSON.stringify(subcategory), attributeName]);
            ...
            `;
            const { rows } = await pool.query(checkQuery, [attributeName, JSON.stringify(category), JSON.stringify(subcategory)]);
            ...
            if (existingAttributeCount === 0) {
                const insertQuery = `
                    ...
                    RETURNING attribute_id;
                `;
                const { rows: attributeRows } = await pool.query(insertQuery, [attributeName, attributeValues, JSON.stringify(category), JSON.stringify(subcategory)]);
        ...
        const categoryIds = backendCategory && backendCategory.map(category => category.category_id);
        ...
        const { rows: categoriesToUpdate } = await pool.query(fetchCategoriesQuery, [categoryIds]);
        ...
        for (const categoryToUpdate of categoriesToUpdate) {
            ...
            await pool.query(updateCategoriesQuery, [categoryToUpdate?.attribute_cat_id, categoryToUpdate?.category_id]);
        }

        // Send success response
        return res.status(200).json({ message: "Data received and inserted/updated successfully" });
